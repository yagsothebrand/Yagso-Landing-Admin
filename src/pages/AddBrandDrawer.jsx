...
import { X, Upload, Building2, CheckCircle2 } from "lucide-react";
...
export function AddBrandDrawer({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await addBrandsItem(formData, user);
      if (res.success) {
              ...
              <DrawerHeader className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 border-b flex-shrink-0 bg-white z-10">
                <div className="flex items-center justify-between gap-2">
                  <DrawerTitle className="text-base sm:text-xl md:text-2xl font-bold text-gray-900 flex items-center gap-2 truncate">
                    <Building2 className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-blue-600 flex-shrink-0" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
                    ...
                    >
                      <div
                        ...
                        className={`aspect-square bg-gradient-to-br from-green-50 to-teal-50 rounded-lg sm:rounded-xl flex items-center justify-center overflow-hidden border-2 border-dashed border-green-300 relative group ${
                          !imagePreview
                            ? "cursor-pointer hover:border-green-500 hover:bg-gradient-to-br hover:from-green-100 hover:to-teal-100 transition-all active:scale-95"
                            : ""
                        }`}
                      >
                        {imagePreview ? (
                          ...
                          <div className="text-center p-4 sm:p-6">
                            <Upload className="w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 text-green-400 mx-auto mb-2 sm:mb-3 md:mb-4" />
